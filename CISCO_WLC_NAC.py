...
def NDA_CHANGE(username,password,counter,config,deploy,device_type,deviceList,outputList,devices,region):
    while not deviceList.empty():
        ...
        try:
            ...
            show_output= connection.send_command_timing('show radius summary').splitlines()
            outputList.put(('\n['+str(counter)+'] BACKUP_CONFIG: ')+str('\n['+str(counter)+'] BACKUP_CONFIG: ').join(show_output))

            # Show Command For WLAN Index Number
            show_output= connection.send_command('show wlan summary').splitlines()
            mgmt_dict = {}
            ...
            if deploy == "yes":
                ...
                if cli_output.__contains__("Command authorization failed"):
                    ...
                    outputList.put('\n['+str(counter)+'] Configuration Completed for '+device)
                    # Remove Local Admin
                    try:
                        # Connect to devices in list
                        outputList.put('\n['+str(counter)+'] Testing authentication and removing Local Admin on '+device)
...
def script(form,dummyarg):
    ...
    config= open('templates/WLC_NAC_config.j2','r').read()
    ...
    subject = 'Results for WLC NAC Changes'
    html = """
    <html>
    <body>
    <h1>Output from WLC NAC Changes</h1>
    </body>
    </html>
    """
